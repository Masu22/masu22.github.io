...
`gallery-dl` で YouTube チャンネルの動画サムネイルを一括ダウンロードすることは **可能** です。
...
- Python（3.x）
- `gallery-dl` のインストール（`pip install gallery-dl`）
...
```bash
gallery-dl --download-archive archive.txt -o output.filename="{title}.{extension}" --skip-metadata https://www.youtube.com/@YourChannelName/videos
```
...
`gallery-dl` はデフォルトでは動画をダウンロードしますが、**`-o skip="content"`** というオプションで **サムネイルとメタデータだけを取得**することが可能です。

サムネイルだけを保存するには、以下のようにします：

```bash
gallery-dl -o skip="content" -o output.filename="{title}.jpg" https://www.youtube.com/@YourChannelName/videos
```
...
- `-o skip="content"`：動画本体はスキップ
- `output.filename="{title}.jpg"`：タイトルをファイル名に
- `--download-archive archive.txt`：重複ダウンロード防止（任意）

---

### 注意点：

- タイトルに使えない文字（例： `\/:*?"<>|`）があるとエラーになるため、自動で処理されます（ただし場合によってはカットされて意図しない名前になるかも）。
- サムネイルは `.jpg` 形式で保存されます。
- チャンネルの動画数が多いと時間がかかります。

---

## .jsonファイルで自動化

必要なら、この用途に合わせて `config.json` も作って自動化できます。  

`config.json` は、`gallery-dl` の**設定ファイル**で、毎回コマンドラインでオプションを指定しなくてもいいようにするためのものです。  

たとえば「常にサムネイルだけ欲しい」「常にファイル名をタイトルにしたい」という設定を、事前に保存しておけるのがこの `config.json` です。

---

### 例：YouTubeのサムネイルだけ保存する用の `config.json`

```json
{
    "extractor": {
        "youtube": {
            "skip": "content"  // 動画はスキップ
        }
    },
    "output": {
        "filename": "{title}.jpg"  // タイトルをファイル名に
    }
}
```
...
- 通常はホームディレクトリの `.config/gallery-dl/config.json` に置きます。

  **Windows** の場合：
  ```
  C:\Users\あなたのユーザー名\gallery-dl\config.json
  ```
...
`config.json` を作ったあとは、コマンドはめっちゃシンプルになります：

```bash
gallery-dl https://www.youtube.com/@YourChannelName/videos
```

→ 自動的に、サムネイルだけがタイトル名で保存される！

---

### 補足：

- `config.json` にさらに細かく保存先ディレクトリなども書けるよ。
- 例えば `directory` オプションでフォルダ分けもできる。
